...
public class Player {
    ...
    NueralNetwork brain;
    ArrayList<Tile> VisistedTiles = new ArrayList<>();
    ...
    public boolean isAlive() {
        return (!goalReached && !collided && !timeout);
    }

    public void setFitness(double fitnes){
        brain.setFitness(fitnes);
    }
    ...
    public boolean isColliding(int tileX, int tileY, int size) {

        double angle = Math.atan2(velocity.getY(), velocity.getX());
    
        // Get the triangle vertices after rotation and translation
        Point2D p1 = getRotatedPoint(size * 2, 0, angle);
        Point2D p2 = getRotatedPoint(-size * 2, -size, angle);
        Point2D p3 = getRotatedPoint(-size * 2, size, angle);
            
        // Translate the points to the triangle's position
    ...
    private boolean isPointInTriangle(Point2D p, Point2D p1, Point2D p2, Point2D p3) {
    ...
    public void steerLeft() {
        ...
        double newAngle = currentAngle - 0.1;
